...
static void button_double_click_cb(void *button_handle, void *usr_data)
{
    ...
    memcpy(&config, &usr_data, sizeof(config));
    ESP_LOGI(TAG, "Config{init_attr : %" PRIi16 ", rssi : %" PRIi8 ", wait_ms : %" PRIu16 ", resp_to_init_ms : %" PRIu16 "}, size of config: %u", config.init_attr, config.rssi, config.wait_ms, config.resp_to_init_ms, sizeof(config));
                                 ...
                                 remote_ctrl_bind_config_t config)
{
    ...
    void *usr_data;
    memcpy(&usr_data, &config, sizeof(void*));

    ESP_RETURN_ON_ERROR(iot_button_register_cb(handle, BUTTON_DOUBLE_CLICK, button_double_click_cb, usr_data), TAG, "Failed to register double click cb");
